# Rails 8 production environment configuration for Rancher UI deployment.
...
#   Set SOLID_QUEUE_IN_PUMA=true in Rancher environment variables
#   Web server + background jobs run in one process
#   Good for: Getting started, low job volume, simplicity
#
# OPTION 2: Separate Worker Workload (Production Scale)
#   Do NOT set SOLID_QUEUE_IN_PUMA (or set to false)
#   Create a second workload in Rancher with command: bin/rails solid_queue:start
...
Rails.application.configure do
  ...
  #
  # For Option 2 (separate workers), create a second Rancher workload:
  ...
  # --- Health Check ---
  # Rails 8 provides /up endpoint by default
  # Rancher probes should hit: http://app:3000/up
end
